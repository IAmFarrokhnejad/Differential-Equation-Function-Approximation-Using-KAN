...
nh=17;                % # of hidden neurons
no=1;                % # of output neurons
ns=21;               % # of samples
m=0.02;              % learning rate
maxep=2000000;        % Maximum Epoch
...
while  (ep_er>0 & n<maxep),
    ...
    wxx=vi+x(j)*y;
    dd=-(x(j)+(1+3*x(j)^2)/(1+x(j)+x(j)^3))*wxx+x(j)^3+2*x(j)+(x(j)^2)*(1+3*x(j)^2)/(1+x(j)+x(j)^3);
    
    del1=y-(2/sig^2)*(x(j)-rng(1))*(x(j)-w1)*(w2.*q)'-dd;
...
ge=(t.^5+t.^3+t.^2+exp(-(t.^2)/2))./(t.^3+t+1);                   % exact fucntion
