...
% Estimation function
fe=@(a1,a2) -2*a2+cos(4*a1);

% Solution
fr=@(a1) 2.9*exp(-2*a1)+0.1*cos(4*a1)+0.2*sin(4*a1); 

rng=[0, 3];      % Range, [min, max]
vi=3;            % Initial value
sig=0.8;         % variance
...
t=rng(1):0.15:rng(2);       % time period
...
CxA=strcat(['ivp_ex2_', num2str(nh), '.mat']);
clearvars -except CxA ResulT MAE MAPE MSE;   
save(CxA);
